...
class Softmax_Cross_Entropy_With_LogitsOp(Op):
	def __call__(self, labels, logits):
		tmp = -reduce_sum(labels * logits, axis = 1, keepdims = True) + reduce_sum(labels, axis = 1, keepdims = True) * log(reduce_sum(exp(logits), axis = 1, keepdims = True))
...
class MaxpoolOp(Op):
	...
	def compute(self, node, input_vals):
		...
		result = np.zeros(shape = (num, n, m, ins), dtype = float32) - 1e35
